...
pub fn string_to_multibyte<S: AsRef<OsStr>>(
    codepage: DWORD,
    data: S,
    default_char: Option<u8>,
) -> Result<Vec<u8>> {
    let wstr: Vec<u16> = data.as_ref().encode_wide().collect();
...
pub fn multi_byte_to_wide_char(
    ...
    unsafe {
        ...
        let len = kernel32::MultiByteToWideChar(
            codepage,
            flags,
...
pub fn wide_char_to_multi_byte(
    ...
    unsafe {
        ...
        let len = kernel32::WideCharToMultiByte(
            codepage,
            flags,
...
fn wide_char_to_multi_byte_empty() {
    assert_eq!(
        wide_char_to_multi_byte(winapi::CP_UTF8, WC_ERR_INVALID_CHARS, &[], None, false).unwrap(),
        (b"".to_vec(), false)
    );
}
